...
def add_wsovod_config(cfg):
    ...
    # RF-DETR
    _C.MODEL.RFDETR = CN()
    _C.MODEL.RFDETR.ENCODER = "dinov2_windowed_small"
    _C.MODEL.RFDETR.OUT_FEATURE_NAMES = ["p3", "p4", "p5", "p6"]
    _C.MODEL.RFDETR.IN_FEATURES = ["p3", "p4", "p5", "p6"]
    _C.MODEL.RFDETR.HIDDEN_DIM = 256
    _C.MODEL.RFDETR.SA_NHEADS = 8
    _C.MODEL.RFDETR.CA_NHEADS = 16
    _C.MODEL.RFDETR.DEC_N_POINTS = 2
    _C.MODEL.RFDETR.NUM_QUERIES = 300
    _C.MODEL.RFDETR.PROJECTOR_SCALE = ["P3", "P4", "P5", "P6"]
    _C.MODEL.RFDETR.OUT_FEATURE_INDEXES = [2, 5, 8, 11]
    _C.MODEL.RFDETR.NUM_CLASSES = 80  # COCO类别数
    _C.MODEL.RFDETR.PRETRAINED_ENCODER = None
    _C.MODEL.RFDETR.WINDOW_BLOCK_INDEXES = None
    _C.MODEL.RFDETR.DROP_PATH = 0.0
    _C.MODEL.RFDETR.USE_CLS_TOKEN = False
    _C.MODEL.RFDETR.POSITION_EMBEDDING = "sine"
    _C.MODEL.RFDETR.FREEZE_ENCODER = False
    _C.MODEL.RFDETR.LAYER_NORM = True
    _C.MODEL.RFDETR.RESOLUTION = 640
    _C.MODEL.RFDETR.RMS_NORM = False
    _C.MODEL.RFDETR.BACKBONE_LORA = False
    _C.MODEL.RFDETR.FORCE_NO_PRETRAIN = False
    _C.MODEL.RFDETR.VIT_ENCODER_NUM_LAYERS = 12
    _C.MODEL.RFDETR.DROPOUT = 0.0
    _C.MODEL.RFDETR.DIM_FEEDFORWARD = 2048
    _C.MODEL.RFDETR.DEC_LAYERS = 3
    _C.MODEL.RFDETR.LITE_REFPOINT_REFINE = True
    _C.MODEL.RFDETR.GROUP_DETR = 1
    _C.MODEL.RFDETR.TWO_STAGE = True
    _C.MODEL.RFDETR.DECODER_NORM = "LN"
    _C.MODEL.RFDETR.BOX_REPARAM = True
    ...
    _C.SOLVER.IMS_PER_BATCH_LIST = [4]

    _C.DATASETS.MIXED_DATASETS = CN()
    _C.DATASETS.MIXED_DATASETS.NAMES = ['coco_2017_train']
    _C.DATASETS.MIXED_DATASETS.WEIGHT_PATH_TRAINS = ['models/coco_text_embedding_single_prompt.pkl']
    _C.DATASETS.MIXED_DATASETS.NUM_CLASSES = [80]
    _C.DATASETS.MIXED_DATASETS.PROPOSAL_FILES = ['']
    _C.DATASETS.MIXED_DATASETS.RATIOS = [1]
    _C.DATASETS.MIXED_DATASETS.USE_CAS = [False]
    _C.DATASETS.MIXED_DATASETS.USE_RFS = [True]
    _C.DATASETS.MIXED_DATASETS.FILTER_EMPTY_ANNOTATIONS = [True]
    _C.DATASETS.MIXED_DATASETS.CAS_LAMBDA = 1.0
    _C.DATASETS.MIXED_DATASETS.REPEAT_THRESHOLD = 0.001